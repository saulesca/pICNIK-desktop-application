...
#-----------------------------------------------------------------------------------------------------------
    def plot_data(self,x_data='time',y_data='TG',x_units='min',y_units='%'):
        ...
        fig, ax = plt.subplots(figsize=(15, 7))  # Guardamos la figura y los ejes
...
#----------------------------------------------------------------
    ...
    def reconstruction(self,E, A, B):
        ...
        for m in range(len(models)):
            ...
            sp2 = sp1.split('<')[1]
            ax.plot(alpha, models[m](alpha, integral=True),
                    ls='--', color=color[m], lw=4.2, label=sp2)
    
        try:
            ax.plot(alpha, g, '*-', color='#6963DB', lw=4.2, ms=8, label='g_r', alpha=0.5)
        except ValueError:
            ax.plot(alpha[1:], g, '*-', color='#6963DB', lw=4.2, ms=8, label='g_r', alpha=0.5)
    
        ax.set_ylim(0, 2)
        ax.set_xlim(0, 1)
        ax.set_xlabel(r'$\alpha$', fontsize=14)
        ax.set_ylabel(r'$g(\alpha)$', fontsize=14)
        ax.grid(True)
        ax.legend(fontsize=12)
        fig.tight_layout()
    
        return fig, np.array(g)
